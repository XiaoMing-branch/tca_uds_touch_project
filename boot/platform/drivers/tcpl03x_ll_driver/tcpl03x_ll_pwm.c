...
typedef union
{
    struct
    {
        uint32_t lt_value : 16;
        uint32_t ht_value : 16;
    };
    uint32_t reg;
} pwm_cfg_reg_t;
...
ll_status_e ll_pwm_channel_threshold_config(pwm_channel_e channel, uint16_t threshold_h, uint16_t threshold_l)
{
    ...
    pwm_cfg_reg_t *regs[] =
    {
        (pwm_cfg_reg_t *) &PWM->CH0_PWM_CFG,
        (pwm_cfg_reg_t *) &PWM->CH1_PWM_CFG,
        (pwm_cfg_reg_t *) &PWM->CH2_PWM_CFG
    };

    regs[channel]->lt_value = threshold_l;
    regs[channel]->ht_value = threshold_h;
